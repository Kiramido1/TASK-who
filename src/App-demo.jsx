...
function App() {
  ...
  const demoParticipants = [
    { id: 'user_1', name: 'أحمد محمد', joinTime: Date.now() - 300000, status: 'active' },
    { id: 'user_2', name: 'فاطمة علي', joinTime: Date.now() - 180000, status: 'active' },
    { id: 'user_3', name: 'محمود حسن', joinTime: Date.now() - 120000, status: 'active' }
  ];
    ...
    if (isConnected) {
      ...
      interval = setInterval(() => {
        setParticipants(prev => 
          prev.map(p => ({
            ...p,
            duration: Math.floor((Date.now() - p.joinTime) / 1000)
          }))
        );
      }, 1000);
    }
  ...
  const handleJoinSegment = async () => {
    ...
    // Simulate joining segment with demo data
    ...
    const allParticipants = [...demoParticipants, newParticipant];
